...
for folder in test_folder:
    ...
    for j in range(int(len(files)*0.1)):
        shutil.move(os.path.join(train_path, files[j]), os.path.join(valid_path, files[j]))
    count += 1
    
# move 30% of trainA directory to testA directory, 30% of trainB directory to testB directory
test_folder = ['testA', 'testB']
...
for folder in test_folder:
    ...
    train_path = os.path.join(new_path, new_folder[count])
    test_path = os.path.join(new_path, folder)
    files = os.listdir(train_path)
    for j in range(int(len(files)/3)):
        shutil.move(os.path.join(train_path, files[j]), os.path.join(test_path, files[j]))
    count += 1
