...
def get_values(method, args, epochs):
    ...
    highest_ssim_A2B_valid = np.max(ssim_A2B_list_valid)
    highest_psnr_A2B_valid = np.max(psnr_A2B_list_valid)
    highest_ssim_B2A_valid = np.max(ssim_B2A_list_valid)
    highest_psnr_B2A_valid = np.max(psnr_B2A_list_valid)
    highest_ssim_A2B_index_valid = np.where(
        ssim_A2B_list_valid == highest_ssim_A2B_valid)
    highest_psnr_A2B_index_valid = np.where(
        psnr_A2B_list_valid == highest_psnr_A2B_valid)
    highest_ssim_B2A_index_valid = np.where(
        ssim_B2A_list_valid == highest_ssim_B2A_valid)
    highest_psnr_B2A_index_valid = np.where(
        psnr_B2A_list_valid == highest_psnr_B2A_valid)
    ...
    return {'lowest_A2B_adversarial_loss_valid': lowest_A2B_adversarial_loss_valid,
            ...
            'lowest_total_loss_valid': lowest_total_loss_valid,
            'lowest_total_loss_valid_index': lowest_total_loss_valid_index,
            ...
            'highest_ssim_A2B_valid': highest_ssim_A2B_valid,
            'highest_psnr_A2B_valid': highest_psnr_A2B_valid,
            'highest_ssim_B2A_valid': highest_ssim_B2A_valid,
            'highest_psnr_B2A_valid': highest_psnr_B2A_valid,
            'highest_ssim_A2B_index_valid': highest_ssim_A2B_index_valid,
            'highest_psnr_A2B_index_valid': highest_psnr_A2B_index_valid,
            'highest_ssim_B2A_index_valid': highest_ssim_B2A_index_valid,
            'highest_psnr_B2A_index_valid': highest_psnr_B2A_index_valid

            }


# Do the same methods, but with all methods
method = ["convolutional", "convolutional_gan",
...
for m in method:
    if m == "convolutional":
    ...
    elif m == "convolutional_gan":
        convolutional_gan_args = get_params(test_args, m)
        convolutional_gan_values = get_values(m, convolutional_gan_args, 1000)
    elif m == "operational":
        ...
        operational_unet_values = get_values(m, operational_unet_args, 850)
...
for key, value in convolutional_gan_values.items():
    output.append(f'Convolutional GAN {key}: {value}')
...
if not os.path.exists(f'output/{test_args.method}'):
    os.makedirs(f'output/{test_args.method}')
output_file = open(
...
G_B2A = module.ResnetGenerator(
    input_shape=(convolutional_args.crop_size, convolutional_args.crop_size, 3))
G_A2B_gan = module.ResnetGenerator(
    input_shape=(convolutional_gan_args.crop_size, convolutional_gan_args.crop_size, 3))
G_B2A_gan = module.ResnetGenerator(
    input_shape=(convolutional_gan_args.crop_size, convolutional_gan_args.crop_size, 3))
...
checkDir_gan = 'output/' + 'convolutional_gan/' + 'checkpoints'
...
def sample_A2B(A):
    ...
    A2B_gan = G_A2B_gan(A, training=False)
    A2B2A_gan = G_B2A_gan(A2B_gan, training=False)
    ...
    return A2B, A2B2A, A2B_gan, A2B2A_gan, A2B_op, A2B2A_op, A2B_unet, A2B2A_unet, A2B_opunet, A2B2A_opunet
...
def sample_B2A(B):
    ...
    B2A_gan = G_B2A_gan(B, training=False)
    B2A2B_gan = G_A2B_gan(B2A_gan, training=False)
    ...
    return B2A, B2A2B, B2A_gan, B2A2B_gan, B2A_op, B2A2B_op, B2A_unet, B2A2B_unet, B2A_opunet, B2A2B_opunet
...
tl.Checkpoint(dict(G_A2B=G_A2B_gan, G_B2A=G_B2A_gan), py.join(
    test_args.experiment_dir, 'convolutional_gan', 'checkpoints')).restore(checkDir_gan +
                                                                           f'/ckpt-{convolutional_gan_values["lowest_A2B_total_loss_valid_index"][0][0]}')
...
for A, B in zip(A_dataset_test, B_dataset_test):
    ...
    #     A)
    A2B, A2B2A, A2B_gan, A2B2A_gan, A2B_op, A2B2A_op, A2B_unet, A2B2A_unet, A2B_opunet, A2B2A_opunet = sample_A2B(
        A)
    ...
    # Adding convolutional gan too
    for A_i, A2B_i, A2B2A_i, A2B_gan_i, A2B2A_gan_i, A2B_op_i, A2B2A_op_i, A2B_unet_i, A2B2A_unet_i, A2B_opunet_i, A2B2A_opunet_i, B_i in zip(A, A2B, A2B2A, A2B_gan, A2B2A_gan, A2B_op, A2B2A_op, A2B_unet, A2B2A_unet, A2B_opunet, A2B2A_opunet, B):
        psnr, ssim = ev.compute_psnr_ssim(A2B_i.numpy(), B_i.numpy())
        psnr_gan, ssim_gan = ev.compute_psnr_ssim(
            A2B_gan_i.numpy(), B_i.numpy())
        ...
        A2B_list = [A2B_i, A2B_gan_i, A2B_op_i, A2B_unet_i, A2B_opunet_i]
        A2B2A_list = [A2B2A_i, A2B2A_gan_i, A2B2A_op_i,
                      A2B2A_unet_i, A2B2A_opunet_i]
        psnr_list = [psnr, psnr_gan, psnr_op, psnr_unet, psnr_opunet]
        ssim_list = [ssim, ssim_gan, ssim_op, ssim_unet, ssim_opunet]
        # method_list = ["convolutional",
        #                "operational", "unet", "operational_unet"]
        method_list = ["convolutional", "convolutional_gan",
                       "operational", "unet", "operational_unet"]
...
tl.Checkpoint(dict(G_A2B=G_A2B_gan, G_B2A=G_B2A_gan), py.join(
    test_args.experiment_dir, 'convolutional_gan', 'checkpoints')).restore(checkDir_gan +
                                                                           f'/ckpt-{convolutional_gan_values["lowest_B2A_total_loss_valid_index"][0][0]}')
tl.Checkpoint(dict(G_A2B=G_A2B_op, G_B2A=G_B2A_op), py.join(
    test_args.experiment_dir, 'operational', 'checkpoints')).restore(checkDir_op +
...
for A, B in zip(A_dataset_test, B_dataset_test):
    ...
    #                            ssim_list, save_dir, A_img_paths_test[i], method_list)
    #     i += 1
    B2A, B2A2B, B2A_gan, B2A2B_gan, B2A_op, B2A2B_op, B2A_unet, B2A2B_unet, B2A_opunet, B2A2B_opunet = sample_B2A(
        B)
    for B_i, B2A_i, B2A2B_i, B2A_gan_i, B2A2B_gan_i, B2A_op_i, B2A2B_op_i, B2A_unet_i, B2A2B_unet_i, B2A_opunet_i, B2A2B_opunet_i, A_i in zip(B, B2A, B2A2B, B2A_gan, B2A2B_gan, B2A_op, B2A2B_op, B2A_unet, B2A2B_unet, B2A_opunet, B2A2B_opunet, A):
        psnr, ssim = ev.compute_psnr_ssim(B2A_i.numpy(), A_i.numpy())
        psnr_gan, ssim_gan = ev.compute_psnr_ssim(
            B2A_gan_i.numpy(), A_i.numpy())
        psnr_op, ssim_op = ev.compute_psnr_ssim(B2A_op_i.numpy(), A_i.numpy())
        psnr_unet, ssim_unet = ev.compute_psnr_ssim(
            B2A_unet_i.numpy(), A_i.numpy())
        psnr_opunet, ssim_opunet = ev.compute_psnr_ssim(
        ...
        # psnr_list = [psnr, psnr_op, psnr_unet, psnr_opunet]
        # ssim_list = [ssim, ssim_op, ssim_unet, ssim_opunet]
        B2A_list = [B2A_i, B2A_gan_i, B2A_op_i, B2A_unet_i, B2A_opunet_i]
        B2A2B_list = [B2A2B_i, B2A2B_gan_i, B2A2B_op_i,
                      B2A2B_unet_i, B2A2B_opunet_i]
        psnr_list = [psnr, psnr_gan, psnr_op, psnr_unet, psnr_opunet]
        ssim_list = [ssim, ssim_gan, ssim_op, ssim_unet, ssim_opunet]
        # method_list = ["convolutional",
        #                "operational", "unet", "operational_unet"]
...
# tl.Checkpoint(dict(G_A2B=G_A2B, G_B2A=G_B2A), py.join(
#     test_args.experiment_dir, 'convolutional', 'checkpoints')).restore(checkDir +
#                                                                        f'/ckpt-{convolutional_values["highest_psnr_A2B_index_valid"][0][0]}')
# tl.Checkpoint(dict(G_A2B=G_A2B_op, G_B2A=G_B2A_op), py.join(
#     test_args.experiment_dir, 'operational', 'checkpoints')).restore(checkDir_op +
#                                                                      f'/ckpt-{operational_values["highest_psnr_A2B_index_valid"][0][0]}')
# tl.Checkpoint(dict(G_A2B=G_A2B_unet, G_B2A=G_B2A_unet), py.join(
#     test_args.experiment_dir, 'unet', 'checkpoints')).restore(checkDir_unet +
#                                                               f'/ckpt-{unet_values["highest_psnr_A2B_index_valid"][0][0]}')
# tl.Checkpoint(dict(G_A2B=G_A2B_opunet, G_B2A=G_B2A_opunet), py.join(
#     test_args.experiment_dir, 'operational_unet', 'checkpoints')).restore(checkDir_opunet +
#                                                                           f'/ckpt-{operational_unet_values["highest_psnr_A2B_index_valid"][0][0]}')
...
# tl.Checkpoint(dict(G_A2B=G_A2B, G_B2A=G_B2A), py.join(
#     test_args.experiment_dir, 'convolutional', 'checkpoints')).restore(checkDir +
#                                                                        f'/ckpt-{convolutional_values["highest_psnr_B2A_index_valid"][0][0]}')
# tl.Checkpoint(dict(G_A2B=G_A2B_op, G_B2A=G_B2A_op), py.join(
#     test_args.experiment_dir, 'operational', 'checkpoints')).restore(checkDir_op +
#                                                                      f'/ckpt-{operational_values["highest_psnr_B2A_index_valid"][0][0]}')
# tl.Checkpoint(dict(G_A2B=G_A2B_unet, G_B2A=G_B2A_unet), py.join(
#     test_args.experiment_dir, 'unet', 'checkpoints')).restore(checkDir_unet +
#                                                               f'/ckpt-{unet_values["highest_psnr_B2A_index_valid"][0][0]}')
# tl.Checkpoint(dict(G_A2B=G_A2B_opunet, G_B2A=G_B2A_opunet), py.join(
#     test_args.experiment_dir, 'operational_unet', 'checkpoints')).restore(checkDir_opunet +
#                                                                           f'/ckpt-{operational_unet_values["highest_psnr_B2A_index_valid"][0][0]}')
...
# tl.Checkpoint(dict(G_A2B=G_A2B, G_B2A=G_B2A), py.join(
#     test_args.experiment_dir, 'convolutional', 'checkpoints')).restore(checkDir +
#                                                                        f'/ckpt-{convolutional_values["highest_ssim_A2B_index_valid"][0][0]}')
# tl.Checkpoint(dict(G_A2B=G_A2B_op, G_B2A=G_B2A_op), py.join(
#     test_args.experiment_dir, 'operational', 'checkpoints')).restore(checkDir_op +
#                                                                      f'/ckpt-{operational_values["highest_ssim_A2B_index_valid"][0][0]}')
# tl.Checkpoint(dict(G_A2B=G_A2B_unet, G_B2A=G_B2A_unet), py.join(
#     test_args.experiment_dir, 'unet', 'checkpoints')).restore(checkDir_unet +
#                                                               f'/ckpt-{unet_values["highest_ssim_A2B_index_valid"][0][0]}')
# tl.Checkpoint(dict(G_A2B=G_A2B_opunet, G_B2A=G_B2A_opunet), py.join(
#     test_args.experiment_dir, 'operational_unet', 'checkpoints')).restore(checkDir_opunet +
#                                                                           f'/ckpt-{operational_unet_values["highest_ssim_A2B_index_valid"][0][0]}')
...
# tl.Checkpoint(dict(G_A2B=G_A2B, G_B2A=G_B2A), py.join(
#     test_args.experiment_dir, 'convolutional', 'checkpoints')).restore(checkDir +
#                                                                        f'/ckpt-{convolutional_values["highest_ssim_B2A_index_valid"][0][0]}')
# tl.Checkpoint(dict(G_A2B=G_A2B_op, G_B2A=G_B2A_op), py.join(
#     test_args.experiment_dir, 'operational', 'checkpoints')).restore(checkDir_op +
#                                                                      f'/ckpt-{operational_values["highest_ssim_B2A_index_valid"][0][0]}')
# tl.Checkpoint(dict(G_A2B=G_A2B_unet, G_B2A=G_B2A_unet), py.join(
#     test_args.experiment_dir, 'unet', 'checkpoints')).restore(checkDir_unet +
#                                                               f'/ckpt-{unet_values["highest_ssim_B2A_index_valid"][0][0]}')
# tl.Checkpoint(dict(G_A2B=G_A2B_opunet, G_B2A=G_B2A_opunet), py.join(
#     test_args.experiment_dir, 'operational_unet', 'checkpoints')).restore(checkDir_opunet +
#                                                                           f'/ckpt-{operational_unet_values["highest_ssim_B2A_index_valid"][0][0]}')
