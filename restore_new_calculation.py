...
import data
import module
from plot import temporary_plot, save_plot_data, save_psnr_and_ssim_data, temporary_plot_psnr_ssim, save_psnr_and_ssim_data_temporary
...
py.arg('--plot_data_dir', default='plot_data_new_calculation')
...
with train_summary_writer.as_default():
    for ep in range(start, ep_step):
        ...
        for A, B in tqdm.tqdm(A_B_dataset_test, desc='Training Epoch Loop', total=len_dataset):
            ...
            psnr_A2B, ssim_A2B = ev.compute_psnr_ssim_mathematical(
                B[0].numpy(), A2B[0].numpy())
            psnr_B2A, ssim_B2A = ev.compute_psnr_ssim_mathematical(
            ...
            summary = {'ssim_A2B_mathematical': ssim_A2B, 'psnr_A2B_mathematical': psnr_A2B,
                       'ssim_B2A_mathematical': ssim_B2A, 'psnr_B2A_mathematical': psnr_B2A}

            # Summary
            tl.summary(summary, step=i_train, name='psnr_ssim_mathematical')
            i_train += 1
        ...
        for A, B in tqdm.tqdm(A_B_dataset_valid, desc='Valid Epoch Loop', total=valid_len_dataset):
            ...
            i_valid += 1
            # Summary
            valid_summary = {'ssim_A2B_valid_mathematical': ssim_A2B, 'psnr_A2B_valid_mathematical': psnr_A2B,
                             'ssim_B2A_valid_mathematical': ssim_B2A, 'psnr_B2A_valid_mathematical': psnr_B2A}
            tl.summary(valid_summary, step=i_valid,
                       name='psnr_ssim_valid_mathematical')

        # Training dataset handling
        save_psnr_and_ssim_data_temporary(iterations, ssim_value_A2B, psnr_value_A2B,
                                          ssim_value_B2A, psnr_value_B2A, ep, "training", args.method)

        # Validation dataset handling
        save_psnr_and_ssim_data_temporary(iterations_valid, ssim_A2B_value_valid, psnr_A2B_value_valid,
